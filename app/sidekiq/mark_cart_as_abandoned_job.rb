class MarkCartAsAbandonedJob
  ...
  def mark_inactive_carts_as_abandoned
    ...
    log_summary("Marked", marked_count, "carts as abandoned")
    marked_count
  end
  ...
  def remove_old_abandoned_carts
    ...
    log_summary("Removed", removed_count, "old abandoned carts")
    removed_count
  end

  def log_summary(action, count, description)
    Rails.logger.info "#{action} #{count} #{description}"
  end
end
